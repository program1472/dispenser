<?php
	...
	function Finish(): void {
		...
		if (!IS_DEBUG) {
			...
			$allowed = ['item', 'items', 'html', 'result', 'pagination', 'dev', 'message', 'msg', 'events', 'totalCount', 'table_array', 'error', 'approval', 'M1', 'PL', 'PR', 'T2', 'T3', 'T4', 'T5', 'T6', 'img', 'ip', 'mb_id', 'tags'];
			...
			// Filter by KEY only (not by value) - keep null/empty values if key is allowed
			$filtered = [];
			foreach ($response as $k => $v) {
				if (in_array($k, $allowed, true) || preg_match('/^[A-Za-z]\d{2}$/', $k)) {
					$filtered[$k] = $v;
				}
			}
			$response = $filtered;
		}
		unset($response['SESSION'],$response['menus']);
