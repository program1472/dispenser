...
require_once __DIR__ . '/common.php'; // 여기서 $menus, $today, 상수/함수 로드됨

$role = $_SESSION['role'] ?? ''; // 'hq' | 'vender' | 'customer' | 'lucid'
...
<head>
	...
	<script>
		function onRoleChange(sel){
		  ...
		  u.pathname = '/dispenser/';                 // 필요 없으면 이 줄 삭제
...
<?php
  ...
  echo '<header>';
  echo '  <div class="brand">'.htmlspecialchars($brand, ENT_QUOTES, 'UTF-8').'</div>';
	...
	if (!empty($menus[$role]) && is_array($menus[$role])) {
		...
		foreach ($menus[$role] as $item) {
			...
			// data-token을 같이 심어두면 초기 로딩 때 토큰으로 정확히 찾아서 active 처리 가능
