...
if (!empty($_POST)) {
  ...
  switch ($action) {
    ...
    case 'VND_NEW_CUSTOMER_SAVE': {
      // VENDOR 포털 정책: 고객 등록은 HQ에서만 가능
      // VENDOR는 고객 정보 조회만 가능 (READ-ONLY)
      $response['result'] = false;
      $response['error']['msg'] = '고객 등록은 HQ 관리자만 가능합니다. 신규 고객 등록이 필요한 경우 HQ에 문의해주세요.';
...
<div class="toolbar row" style="justify-content:space-between;margin-bottom:8px">
  ...
  <div class="row">
    ...
    <button class="btn" id="btnNewCust" disabled title="고객 등록은 HQ 관리자만 가능합니다">신규 고객 등록 (HQ 전용)</button>
  </div>
</div>
...
(function(){
  ...
  function bind(){
    ...
    // 신규 버튼 - VENDOR는 고객 등록 불가 (HQ 전용)
    $('#btnNewCust')?.addEventListener('click', ()=>{
      toast('고객 등록은 HQ 관리자만 가능합니다. 신규 고객 등록이 필요한 경우 HQ에 문의해주세요.');
    });
