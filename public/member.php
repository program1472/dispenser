...
function require_role($code){
    ...
    $sql = "INSERT INTO roles (code,name".(has_col('roles','created_at')?",created_at":"").") VALUES ('".j($code)."','".j($code)."'".(has_col('roles','created_at')?(",".sqlv(now())):"").")";
...
if ($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        ...
        if ($type==='VENDOR'){
            ...
            $sqlV=''; insert_row('vendors', $vendorRow, $sqlV);
            $response['data']['reg']['sqlV'] = $sqlV;
            $vendorRef = $vid;
            $extra_create['vendor_id']=$vid;
        }
        ...
        if ($type==='CUSTOMER'){
            ...
            $customerRow = [
                ...
                'vendor_id'       => has_col('customers','vendor_id') ? $vendorRef : null,
                'contract_state'  => has_col('customers','contract_state') ? '활성' : null,
            ...
            $sqlC=''; insert_row('customers', $customerRow, $sqlC);
            $response['data']['reg']['sqlC'] = $sqlC;

            // 주소가 있으면 기본 사이트
            if (($address!=='' || $detailaddress!=='') && table_columns('sites')){
                $sid = make_id('S');
                $siteRow = [
                    'site_id'     => $sid,
                    'customer_id' => $cid,
                    'name'        => has_col('sites','name') ? '본점' : null,
                    'address'     => has_col('sites','address') ? trim($address.' '.$detailaddress) : null,
                    'created_at'  => has_col('sites','created_at') ? now() : null,
                ];
                $sqlS=''; insert_row('sites', $siteRow, $sqlS);
                $response['data']['reg']['sqlS'] = $sqlS;
                $extra_create['site_id']=$sid;
            }
            $customerRef = $cid;
            $extra_create['customer_id']=$cid;
        }
        ...
        $userRow = [
            ...
            'phone'         => $hphone?:$phone,
            'role_id'       => $roleId,
            'vendor_id'     => has_col('users','vendor_id') ? $vendorRef : null,
            'customer_id'   => has_col('users','customer_id') ? $customerRef : null,
