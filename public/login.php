<?php
	...
	/* 사용자 조회(이메일 또는 아이디) */
	/* 사용자 조회(이메일) — 새 스키마: users ← vendors/customers(user_id FK) */
	function find_user($identifier, $mode){ // $mode: email | id
		global $con;
		$identifier = trim($identifier);

		// password 컬럼명 감지 (password_hash 또는 password)
		$password_col = has_col('users','password_hash') ? 'u.password_hash' : 'u.password';

		// roles 테이블: code 또는 role_name 컬럼 감지
		$role_code_col = has_col('roles','code') ? 'r.code' : 'UPPER(r.role_name)';

		// vendors, customers는 user_id FK를 가짐 (역참조)
		$select = "
			u.user_id, u.email, {$password_col} AS password_hash, u.name,
			".(has_col('users','phone')?'u.phone':'NULL AS phone').",
			u.role_id,
			v.vendor_id,
			c.customer_id,
			".(has_col('users','is_active')?'u.is_active':'1 AS is_active').",
			{$role_code_col} AS role_code,
			".(has_col('roles','role_name')?'r.role_name':(has_col('roles','name')?'r.name':'r.role_name'))." AS role_name
		";

		if ($mode === 'email') {
			// 이메일로 찾기 (vendors, customers LEFT JOIN으로 FK 역참조)
			$sql = "SELECT {$select}
					FROM users u
	...
	/* ---------- 입력 복호화 파이프 ---------- */
	$raw_input = ''; // Store raw input for debugging
	...
	if ($_SERVER['REQUEST_METHOD']==='POST'){
		...
		// Ensure $_POST is always an array
	...
	if ($_SERVER['REQUEST_METHOD']==='POST'){
		try{
			...
			error_log("POST Data after decrypt: " . json_encode($_POST, JSON_UNESCAPED_UNICODE));
			error_log("POST[email]: " . var_export($_POST['email'] ?? 'NOT SET', true));
			error_log("POST[password]: " . var_export($_POST['password'] ?? 'NOT SET', true));
			...
			if ($identifier === '') throw new Exception('이메일을 입력하세요. POST: ' . json_encode($_POST, JSON_UNESCAPED_UNICODE));
			...
			$menu_key = $role_to_menu_key[$role] ?? 'customer';

			/* 세션 — 새 스키마: vendor_id, customer_id는 LEFT JOIN 결과에서 가져옴 */
			...
			$_SESSION['user'] = [
				...
				'vendor_id'   => $user['vendor_id'] ?? null,      // LEFT JOIN vendors
				'customer_id' => $user['customer_id'] ?? null,    // LEFT JOIN customers
			...
			$_SESSION['menu'] = $menus[$menu_key] ?? null;
			$payload = [
				...
				'route_rule' => [                        // 4. 메뉴 클릭 규칙
					...
					'menu'  => $menus[$menu_key] ?? null	//   메뉴 아이디 기반 로딩 규칙
				],
			];
